...
const ModalRedactarInformativo: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  return (
    <ScrollReveal className="fixed inset-0 bg-black/80 backdrop-blur-sm z-110 flex justify-center items-center p-4 animate-fadeIn">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl relative flex flex-col max-h-[90vh] reveal-element delay-300">
        <div className="bg-blue-200 p-6 pr-12 relative shrink-0 rounded-t-2xl">
          ...
          <h2 className="text-white font-extrabold text-[1.4rem] leading-tight">Redactar / Editar Informativo</h2>
        ...
        <div className="p-6 md:p-8 overflow-y-auto no-scrollbar bg-gray-50 flex-1">
          <form className="space-y-5">
            <div>
              ...
              <input type="text" className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-blue-200 text-[0.95rem] outline-none focus:border-orange-500" />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea rows={4} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-blue-200 text-[0.90rem] outline-none resize-none focus:border-orange-500"></textarea>
            </div>
            <div>
              ...
              <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-lg p-2">
                <input type="file" className="w-full text-[0.8rem] text-gray-500 file:mr-4 file:py-1.5 file:px-4 file:rounded-md file:border-0 file:bg-blue-200 file:text-white hover:file:bg-orange-500 cursor-pointer" />
              </div>
            </div>
          </form>
        </div>
        <div className="p-6 border-t border-gray-100 bg-white shrink-0 flex gap-3">
          <button onClick={onClose} className="flex-1 py-3.5 border border-gray-200 rounded-md cursor-pointer text-gray-600 font-bold uppercase text-[0.80rem] hover:bg-gray-50">Cancelar</button>
          <button className="flex-1 py-3.5 bg-orange-500 text-white rounded-md font-bold cursor-pointer uppercase text-[0.80rem] hover:bg-blue-200 shadow-md transition-all">Guardar y Publicar</button>
        </div>
      </div>
    </ScrollReveal>
  );
