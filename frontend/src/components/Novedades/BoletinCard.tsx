...
export const BoletinCard = ({ id, fecha, image, titulo, content }: boletinesProps) => {
    
    // Función para procesar la URL de la imagen
    const obtenerUrlImagen = (ruta: string | null) => {
        if (!ruta) return '/images/Recurso04.jpeg'; // Imagen por defecto si no subieron portada
        if (ruta.startsWith('http')) return ruta; // Por si ya viene la URL completa
        
        // Asumiendo que tu backend de Laravel está en VITE_BACKEND_URL (ej. http://localhost:8000)
        // Asegúrate de tener tu variable de entorno configurada, o cambia el string directamente.
    ...
    return (
        <ScrollReveal>
            ...
            <div className="p-8 flex flex-col grow relative z-20 -mt-4">
                ...
                <Link to={`/novedades-sub`} className="inline-flex items-center gap-2 text-white font-medium text-sm group/link hover:text-orange-500 transition-colors">
